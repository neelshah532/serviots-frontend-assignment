...
export const SkeletonLoader = ({ type, count = 1 }: ISkeletonLoaderProps) => {
  if (type === 'table') {
    return (
      <div className="w-full overflow-x-auto rounded-(--radius-lg) bg-(--color-surface) shadow-(--shadow-sm)">
        <table className="w-full text-left whitespace-nowrap">
          ...
          <tbody>
            {Array.from({ length: count }).map((_, i) => {
              return (
                <tr key={i} className="h-[56px] border-b border-(--color-border-subtle) last:border-b-0">
                  ...
                  <td className="px-6 py-3"><div className="h-5 w-14 rounded-(--radius-sm) shimmer" /></td>
                  <td className="px-6 py-3 text-right"><div className="flex justify-end gap-2"><div className="h-6 w-6 rounded-(--radius-sm) shimmer" /><div className="h-6 w-6 rounded-(--radius-sm) shimmer" /><div className="h-6 w-6 rounded-(--radius-sm) shimmer" /></div></td>
                </tr>
              );
            })}
          </tbody>
  ...
  if (type === 'card') {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {Array.from({ length: count }).map((_, i) => {
          return (
            <div key={i} className="bg-(--color-surface) rounded-(--radius-xl) shadow-(--shadow-sm) overflow-hidden">
              <div className="p-6 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2.5 pt-4 border-t border-(--color-border-subtle)"><div className="h-3 w-4/5 rounded shimmer" /><div className="h-3 w-2/3 rounded shimmer" /><div className="h-3 w-3/4 rounded shimmer" /></div>
              </div>
              <div className="h-10 border-t border-(--color-border) shimmer" />
            </div>
          );
        })}
      </div>
  ...
  if (type === 'detail') {
    return (
      <div className="w-full max-w-4xl mx-auto space-y-10">
        <div className="flex items-center gap-6 pb-8 border-b border-(--color-border)">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {Array.from({ length: 4 }).map((_, si) => {
            return (
              <div key={si} className="bg-(--color-surface) rounded-(--radius-xl) p-7 shadow-(--shadow-sm)">
                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-(--color-border-subtle)"><div className="w-8 h-8 rounded-(--radius-md) shimmer" /><div className="h-4 w-28 rounded shimmer" /></div>
